package frc.robot;

public final class Constants {
  //Constants
  public static final int kJoystickPort = 0;

  //Elevator Constants
  public static final int kElevatorMotor1 = 2;
  public static final int kElevatorMotor2 = 4;
  public static final int kElevatorLimitSwitchBottom = 0;
  public static final int kElevatorLimitSwitchTop = 0;
  public static final double kMaxElevator = 10;
  
  //InOutTake Constants
  public static final int kInOutTakeMotor = 5;
  public static final int kInOutTakeElevatorMotor = 3;
  public static final int kInOutTakeRotationMotor = 5;
  public static final int kInOutTakeColorSensor = 0;
  public static final int kInOutTakeLimitSwitchElevator = 0;
  public static final int kInOutTakeLimitSwitchTop = 0;
  public static final int kInOutTakeLimitSwitchBottom = 0;
  public static final double kMaxInOutTakeElevator = 10;
  public static final double kMaxInOutTakeRotation = 10;
}